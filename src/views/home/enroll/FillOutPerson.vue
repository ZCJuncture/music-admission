    ...
    .wp-card
      el-card.card-person
        ...
        el-form(label-suffix=":" size="small")
          ...
          el-form-item(label="出生日期" label-width="90px")
            ...
            span(v-else) {{model.birthday | date}}
          ...
          el-form-item(label="省联考号" label-width="90px")
            el-input(v-if="editable" v-model="model.provinceExamNo" placeholder="无省艺术联考可填0")
            span(v-else) {{model.provinceExamNo}}

          el-form-item(label="生源地" label-width="60px")
            el-cascader(v-if="editable" v-model="model.hometown" :options="cityList" :props="{value: 'label'}")
            span(v-else) {{model.hometown[0] + ' ' + model.hometown[1]}}

          el-form-item(label="毕业院校" label-width="90px")
            el-input(v-if="editable" v-model="model.school")
            span(v-else) {{model.school}}

          el-form-item(label="高考考号" label-width="90px")
            el-input(v-if="editable" v-model="model.collegeExamNo")
      ...
      el-card.card-contact
        ...
        el-form(label-suffix=":" size="small")
          ...
          el-form-item(label="邮政编码" label-width="90px")
            el-input(v-if="editable" v-model="model.postcode")
            span(v-else) {{model.postcode}}

          el-form-item(label="邮寄地址" label-width="75px" style="width: 100%")
...
export default class FillOutPerson extends Vue {
  ...
  public removeFamily(index: number) {
